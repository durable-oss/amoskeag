...
pub fn transpile(expr: &Expr, config: &TranspileConfig) -> Result<String, TranspileError> {
    ...
    // Add header comment
    writeln!(&mut output, "// Generated by Amoskeag JavaScript Transpiler")
...
fn generate_helpers(output: &mut String, indent: &str) -> Result<(), TranspileError> {
    ...
    .map_err(|e| TranspileError::FormatError(e.to_string()))?;
    writeln!(output, "{}if (val === null || val === undefined) {{", indent)
        .map_err(|e| TranspileError::FormatError(e.to_string()))?;
...
fn transpile_expr(expr: &Expr, indent: &str, depth: usize) -> Result<String, TranspileError> {
    match expr {
        ...
        Expr::FunctionCall { name, args } => {
            ...
            match name.as_str() {
                ...
                "capitalize" => Ok(format!("({}.charAt(0).toUpperCase() + {}.slice(1).toLowerCase())", arg_codes[0], arg_codes[0])),
                "strip" => Ok(format!("{}.trim()", arg_codes[0])),
                "split" => Ok(format!("{}.split({})", arg_codes[0], arg_codes[1])),
                "join" => Ok(format!("{}.join({})", arg_codes[0], arg_codes[1])),
                "truncate" => Ok(format!("{}.substring(0, Math.floor({}))", arg_codes[0], arg_codes[1])),
  ...
  return Math.round(({}) * factor) / factor;
                ...
                "size" => Ok(format!("({} !== null && {} !== undefined ? {}.length : 0)", arg_codes[0], arg_codes[0], arg_codes[0])),
                "first" => Ok(format!("({} && {}.length > 0 ? {}[0] : null)", arg_codes[0], arg_codes[0], arg_codes[0])),
                "last" => Ok(format!("({} && {}.length > 0 ? {}[{}.length - 1] : null)", arg_codes[0], arg_codes[0], arg_codes[0], arg_codes[0])),
                "contains" => Ok(format!("({} !== null && {} !== undefined ? {}.includes({}) : false)", arg_codes[0], arg_codes[0], arg_codes[0], arg_codes[1])),
                "sum" => Ok(format!("({} !== null && {} !== undefined ? {}.reduce((a, b) => a + b, 0) : 0)", arg_codes[0], arg_codes[0], arg_codes[0])),
                "avg" => Ok(format!("({} && {}.length > 0 ? {}.reduce((a, b) => a + b, 0) / {}.length : null)", arg_codes[0], arg_codes[0], arg_codes[0], arg_codes[0])),
                "sort" => Ok(format!("({} !== null && {} !== undefined ? [...{}].sort() : [])", arg_codes[0], arg_codes[0], arg_codes[0])),
                "keys" => Ok(format!("({} !== null && {} !== undefined ? Object.keys({}) : [])", arg_codes[0], arg_codes[0], arg_codes[0])),
                "values" => Ok(format!("({} !== null && {} !== undefined ? Object.values({}) : [])", arg_codes[0], arg_codes[0], arg_codes[0])),
                "reverse" => Ok(format!("({} !== null && {} !== undefined ? [...{}].reverse() : [])", arg_codes[0], arg_codes[0], arg_codes[0])),
                "at" => Ok(format!("({} && Math.floor({}) < {}.length ? {}[Math.floor({})] : null)", arg_codes[0], arg_codes[1], arg_codes[0], arg_codes[0], arg_codes[1])),
                ...
                "if_then_else" => Ok(format!("(_isTruthy({}) ? {} : {})", arg_codes[0], arg_codes[1], arg_codes[2])),
                ...
                "is_nil" => Ok(format!("({} === null || {} === undefined)", arg_codes[0], arg_codes[0])),
                "is_array" => Ok(format!("Array.isArray({})", arg_codes[0])),
                "is_dictionary" => Ok(format!("(typeof {} === 'object' && {} !== null && !Array.isArray({}))", arg_codes[0], arg_codes[0], arg_codes[0])),
                "coalesce" => Ok(format!("({} !== null && {} !== undefined ? {} : {})", arg_codes[0], arg_codes[0], arg_codes[0], arg_codes[1])),
                "default" => Ok(format!("({} !== null && {} !== undefined ? {} : {})", arg_codes[0], arg_codes[0], arg_codes[0], arg_codes[1])),
        ...
        Expr::If {
            ...
            Ok(format!("(_isTruthy({}) ? {} : {})", cond_code, then_code, else_code))
        }
        ...
        Expr::Binary { op, left, right } => {
            ...
            match op {
                ...
                BinaryOp::And => Ok(format!("(_isTruthy({}) && _isTruthy({}))", left_code, right_code)),
                BinaryOp::Or => Ok(format!("(_isTruthy({}) || _isTruthy({}))", left_code, right_code)),
            }
        }
